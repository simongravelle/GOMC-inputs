...
def estimate_the_pressure_H2O():
    # the only required argument is mu_K
    # mu_K is assumed to be provided as a positive number
    # mu_K is assumed to be in K

    mu_K = np.float32(sys.argv[2])*ureg.kelvin
...
def update_pressure_in_input():
    P_bar_CO2 = estimate_the_pressure_CO2()
    P_bar_H2O = estimate_the_pressure_H2O()
    initial_file = open("input.gomc", "r")
    all_lines = []
    for line in initial_file:
        if "Fugacity CO2" in line:
            line = "Fugacity CO2 "+str(P_bar_CO2.magnitude)+"\n"
        if "Fugacity H2O" in line:
            line = "Fugacity H2O "+str(P_bar_H2O.magnitude)+"\n"
